...
import type { HajjOpsGroup } from '@/lib/hajj-ops-types';

interface HajjPackageOption {
  id: string;
  name: string;
}
...
export default function HajjGroupsPage() {
  ...
  const [packages, setPackages] = useState<HajjPackageOption[]>([]);
  ...
  function load() {
    apiRequest<HajjOpsGroup[]>('/hajj-ops/hajj-groups')
      .then(setGroups)
      .catch((err) => setError(err instanceof ApiError ? err.message : 'Failed to load groups'));
    apiRequest<HajjPackageOption[]>('/hajj/packages/admin').then(setPackages).catch(() => undefined);
  }
  ...
  async function handleCreate(e: FormEvent) {
    ...
    setError(null);
    try {
      await apiRequest('/hajj-ops/hajj-groups', {
        method: 'POST',
        body: {
          name,
  ...
  return (
    <ProtectedRoute allowedRoles={['SUPER_ADMIN', 'COMPANY_ADMIN', 'BRANCH_MANAGER', 'STAFF']}>
      <AppShell title="Hajj Groups" navLinks={ADMIN_NAV}>
        <h2 className="text-lg font-semibold text-slate-900">Hajj Group Management</h2>
              ...
              {groups?.map((g) => (
                <tr key={g.id} className="hover:bg-slate-50">
                  <td className="px-3 py-2 font-medium text-slate-800">
                    <Link href={`/admin/hajj-groups/${g.id}`} className="hover:underline">
                      {g.groupNumber}
                    </Link>
                  </td>
