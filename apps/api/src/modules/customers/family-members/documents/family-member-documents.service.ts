...
export class FamilyMemberDocumentsService {
  ...
  async getDocument(documentId: string, ownerMemberId?: string) {
    ...
    if (ownerMemberId && document.familyMemberId !== ownerMemberId) {
      throw new ForbiddenException('This document does not belong to this family member');
    }
    return document;
  }
  ...
  async deleteDocument(documentId: string, ownerMemberId?: string) {
    ...
    await this.prisma.familyMemberDocument.delete({ where: { id: document.id } });
    await fs.promises
      .unlink(documentFilePath(FAMILY_MEMBER_DOCUMENTS_NAMESPACE, document.storedFileName))
      .catch(() => undefined);
  }
}
